...
MochaFlow lets you customize Streamlit previews in two ways:
...
report = Report("Config Demo").configure_streamlit(
    page_title="MochaFlow — Config Demo",
...
- If a hook raises an exception, MochaFlow continues rendering (and logs a gentle warning in the UI).
...
For full control, pass `css="..."` with your own `<style>` rules — MochaFlow injects them safely after computed rules.
