...
EasyPour keeps the numbering consistent in Markdown, PDF, Streamlit, and Dash. If a label does not exist, `report.ref()` raises a `KeyError` (or returns the optional `default=` string).
